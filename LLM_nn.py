...
class LLMNeuralNetwork:
    def __init__(self, dims, activation='sigmoid', lr=0.01):
        self.dims = dims
        self.lr = lr
        self.params = {}
    ...
    def forward(self, X):
        ...
        for i in range(1, len(self.dims)):
            ...
            cache[f'Z{i}'], cache[f'A{i}'] = Zi, Ai
            A = Ai
        # Softmax output
        ZL = A.dot(self.params[f'W{len(self.dims)-1}']) + self.params[f'b{len(self.dims)-1}']
        exp = np.exp(ZL - np.max(ZL, axis=1, keepdims=True))
        AL = exp/exp.sum(axis=1, keepdims=True)
        cache['ZL'], cache['AL'] = ZL, AL
        return AL, cache
